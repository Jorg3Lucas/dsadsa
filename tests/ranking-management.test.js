...
describe('Allied Clans Pagination - Discord 25-option limit', () => {
    ...
    it('should not exceed 25 options when viewing world with many clans', async () => {
        // Create 50 clans for world '1' (the first configured world)
        const clans = Array.from({ length: 50 }, (_, i) => `Clan ${i + 1}`);
        db.config.alliedClans['1'] = clans;

        const interaction = createMockInteraction('manage_allied');
        
        await handleManageAllied(interaction, db, () => {}, () => {});
    ...
    it('should deny access for non-admin users', async () => {
        const interaction = createMockInteraction('manage_allied', [], false);
