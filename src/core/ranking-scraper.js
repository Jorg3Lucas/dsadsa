...
/**
 * Fetch ranking data for ALL configured worlds (single-server mode: NA42).
 *
 * Returns: { "553": { "PlayerName": "ClanName", ... } }
 */
export async function fetchMir4RankingData(forceRefresh = false) {
    ...
    const worldEntries = Object.entries(WORLD_IDS);
    console.log(`\n🚀 Starting ranking scrape for ${worldEntries.length} server(s)...`);
    logMemory('Scrape Start');

    // Create tasks for all configured worlds (only NA42 today)
    const tasks = worldEntries.map(([worldId]) => () => {
    ...
    // Merge results into final result
    const result = {};
    let totalPlayers = 0;
    ...
    console.log(`\n🎉 SCRAPE COMPLETE!`);
    console.log(`   Total: ${totalPlayers} players across ${worldEntries.length} world(s) in ${totalElapsed}s`);
