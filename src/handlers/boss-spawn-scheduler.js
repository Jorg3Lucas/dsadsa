...
export async function sendBossSpawnAlerts() {
  // Uses the channel configured with !setreminders
  const channel = await resolveAlertChannel(dailyLogs.bossSpawnChannelId);
  if (!channel) return;
...
export async function sendCustomWorldBossAlerts(now = getLocalTime()) {
  ...
  // Uses the channel configured with !setevents
  const channel = await resolveAlertChannel(dailyLogs.scheduledEventChannelId);
  if (!channel) return;
